...
[변경 사항]
- Bull/Bear의 system_prompt에 "도구 사용 안내" 섹션 추가.
  Moderator system_prompt는 그대로 (Moderator는 tool 사용 안 함).
"""
...
# ═════════════════════════════════════════════════════════
# 도구 사용 안내 (Bull/Bear 공통)
# ═════════════════════════════════════════════════════════
TOOL_USAGE_GUIDE = """
[도구 사용 안내]
제공된 데이터로 주장을 뒷받침하기 부족하다고 판단되면 다음 도구를 호출할 수 있습니다.
- search_web_news: DB에 없는 최신 뉴스 (최근 발표·이슈 확인용, 네이버 뉴스)
- search_more_articles: DB에서 다른 키워드로 추가 검색 (경쟁사·업종 동향 등)
충분한 데이터가 이미 있다면 도구 호출 없이 바로 주장을 작성하세요.
도구는 꼭 필요할 때만 호출하며, 동일 쿼리를 반복 호출하지 마세요."""
